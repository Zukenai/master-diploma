# Roadmap

## Iteration 01 — MVP Baseline Scaffold

Goal:

- deliver a runnable local baseline with planning artifacts, sample data, sparse retrieval, rule-based scoring, explanation, CLI, docs, and tests

Status:

- completed

## Iteration 02 — Retrieval Upgrade

Goal:

- improve retrieval architecture without breaking the MVP baseline

Planned focus:

- retriever interfaces stabilization
- dense retriever stub or initial implementation
- hybrid fusion design
- reranker interface
- better config management
- improved evidence model

Status:

- completed

## Iteration 03 — Balanced Architecture and Evaluation Harness

Goal:

- reach a thesis-ready architecture suitable for core experimental description

Planned focus:

- dense retrieval
- hybrid retrieval
- reranking
- improved scoring features
- claim-aware overlap logic
- evaluation harness
- baseline comparisons

Status:

- completed

## Iteration 04 — Core Experimental Setup

Goal:

- move from toy-only setup to a small curated real scholarly corpus with reproducible acquisition and thesis-grade experimental support

Planned focus:

- curated scholarly corpus
- provenance and acquisition workflow
- local integration into indexing and retrieval
- repeatable evaluation on the curated setup
- sparse/dense/hybrid comparison artifacts
- final constrained calibration and curated-control strengthening pass
- final evidence-state verdict calibration pass with ordinal diagnostics

Status:

- completed as the final core development phase

## Next Phase — Thesis-Oriented Packaging

Scope:

- experiment presentation
- chapter-friendly summaries
- reproducibility framing
- appendix/support materials

Status:

- remaining work after core development stop
